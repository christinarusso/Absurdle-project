...
"""
def solver3Updated(biggestBucketWords, prevGuesses, nOfSolutionWords, index, console):
    ...
    for word in biggestBucketWords :
        ...
        if (index == (nOfSolutionWords - 2)) : # will look two steps ahead if the current index (indicating how many guesses have already been made on the route)
            ...
            if not allBucketsAreLengthOne(buckets) : # if looking at the next step, not all the buckets are length one, then it is a dead end
                ...
                subroutes = solver3Updated(newBiggestBucketWords1, newPrevGuesses, nOfSolutionWords, index + 1, console) # recursively creates solution routes if they exist 
                #and the index is bumped up one as a new guess has been made 
                routes.extend(subroutes) # all solutions are added to routes 
    return routes # return all solutions 

"""
This will return all possible solution sequences of words which have a length of nOfSolutionWords. 
"""
def solver3helper(wordlist, nOfSolutionWords, console) :
    ...
    for word in wordlist: # iterates through all the words in the wordlist and finds solution sequences of words starting with that word if they exist
        ...
        routes = solver3Updated(biggestBucketWords, [word], nOfSolutionWords, 1, console) # routes will contain all solution sequences of words with the same starting guess word
...
def main(wordListFile):
    ...
    while True:
        routes = solver3helper(wordList, nOfSolutionWords, console) #kept console for debugging
...
def getMinLengthRoutes(wordList):
    ...
    while minLengthRoutes == []:
        routes = solver3helper(wordList, nOfSolutionWords, console) 
        if routes != [] :
